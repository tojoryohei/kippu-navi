...
export async function GET(request: Request) {
  ...
  const searchType = searchParams.get("searchType") || "ticket";
  const isIc = searchParams.get("isIc") === "true";

  if (!from || !to) {
    return NextResponse.json({ error: "出発駅と到着駅を指定してください。" }, { status: 400 });
  }

  if (from === to) {
    return NextResponse.json({ error: "出発駅と到着駅が同じです。" });
  }

  if (!STATION_NAMES.has(from) || !STATION_NAMES.has(to)) {
    return NextResponse.json({ error: "駅名が正しくありません。正しい駅名を選択または入力してください。" });
  }
  ...
  if (isIc) {
    ...
    if (!fromStation?.icPassAreaName || !toStation?.icPassAreaName) {
      return NextResponse.json({ error: "出発駅と到着駅の両方がIC定期券エリア（Suicaエリアなど）の駅である必要があります。" });
    }

    if (fromStation.icPassAreaName !== toStation.icPassAreaName) {
      return NextResponse.json({ error: `異なるIC定期券エリア（${fromStation.icPassAreaName}と${toStation.icPassAreaName}）をまたぐ計算はできません。` });
    }
  }
  ...
  if (!isIc && (searchType === "normal" || searchType === "ticket")) {
    try {
      ...
      if (!cacheResult) return NextResponse.json({ error: "指定された区間の経路が見つかりませんでした。" });
      return NextResponse.json({ result: cacheResult.data, serverTime: cacheResult.time });
    } catch (err: unknown) {
  ...
  if (TEMPORARY_STATIONS.toString().includes(from) || TEMPORARY_STATIONS.toString().includes(to)) {
    return NextResponse.json({ error: "臨時駅発着の定期券は計算できません。" });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "定期券の計算APIとの通信に失敗しました。" });
  }
}
