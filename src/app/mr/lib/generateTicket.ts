...
import { calcFare } from '@/components/calcFare';
...
class TicketGenerator {
    public processRouteAndCalculateFare(request: RouteRequest): ApiResponse {
        ...
        // 経路の補正
        const [correctedPath, countKitashinchi] = correctPath.correctPath(fullPath);
        ...
        const fare = calcFare.calculateFareFromCorrectedPath(correctedPath)
            + calcFare.calculateBarrierFreeFeeFromCorrectedPath(correctedPath);
...
export const generateTicket = new TicketGenerator();