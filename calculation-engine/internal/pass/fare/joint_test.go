...
func TestCalculateJointFare(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: func() int {
				// 10kmの基準額(1ヶ月) 5940
				standard10_1, _ := standardCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeTrunkOnly, EigyoKilo: 100, GiseiKilo: 100, Months: 1})
				return standard10_1
			}(),
		},
		{
			...
			want: func() int {
				...
				standard37_6, _ := standardCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeTrunkOnly, EigyoKilo: 368, GiseiKilo: 368, Months: 6})
				east21_6, _ := eastCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeTrunkOnly, EigyoKilo: 207, GiseiKilo: 207, Months: 6})
				standard21_6, _ := standardCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeTrunkOnly, EigyoKilo: 207, GiseiKilo: 207, Months: 6})
		...
		},
		{
			...
			want: func() int {
				...
				standard99_3, _ := standardCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeMixed, EigyoKilo: 916, GiseiKilo: 990, Months: 3})
				east91_3, _ := eastCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeMixed, EigyoKilo: 834, GiseiKilo: 904, Months: 3})
				standard91_3, _ := standardCalc.Calculate(passdomain.PassFareParams{RouteType: domain.RouteTypeMixed, EigyoKilo: 834, GiseiKilo: 904, Months: 3})
