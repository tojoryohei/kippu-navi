...
func TestNewIcPassGraph(t *testing.T) {
	t.Run("正常系: ICエリアのみ抽出され、IDマッパーが共有されること", func(t *testing.T) {
		...
		// A-B間はIC対応
		g.AddEdge(passdomain.PassEdge{Edge: domain.Edge{FromID: idA, ToID: idB, GiseiKilo: 100}, IsIcPassArea: true})
		// B-C間はIC非対応
		g.AddEdge(passdomain.PassEdge{Edge: domain.Edge{FromID: idB, ToID: idC, GiseiKilo: 200}, IsIcPassArea: false})
	...
	t.Run("正常系: BFSによる連結成分の特定が正しく行われること", func(t *testing.T) {
		...
		g.AddEdge(passdomain.PassEdge{Edge: domain.Edge{FromID: idA, ToID: idB}, IsIcPassArea: true})
		g.AddEdge(passdomain.PassEdge{Edge: domain.Edge{FromID: idB, ToID: idA}, IsIcPassArea: true}) // 双方向

		g.AddEdge(passdomain.PassEdge{Edge: domain.Edge{FromID: idC, ToID: idD}, IsIcPassArea: true})
		g.AddEdge(passdomain.PassEdge{Edge: domain.Edge{FromID: idD, ToID: idC}, IsIcPassArea: true}) // 双方向
	...
	t.Run("異常系: 駅数とエッジスライスのサイズが不一致（データ破損）の場合はエラーとなること", func(t *testing.T) {
		...
		g.Edges = make([][]passdomain.PassEdge, 1) // 駅数は3だが、Edgesはサイズ1しかない状態
