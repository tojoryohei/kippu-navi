...
type TopologyProvider interface {
	GetEdges(stationID int) []passdomain.PassEdge
}
...
func (g *FastGraph) GetEdges(id int) []passdomain.PassEdge {
...
func (g *FastGraph) AddEdge(edge passdomain.PassEdge) {
	// 必要に応じてスライスを拡張
	maxID := edge.FromID
	if edge.ToID > maxID {
		maxID = edge.ToID
	...
	if maxID >= len(g.Edges) {
		newEdges := make([][]passdomain.PassEdge, maxID+1)
