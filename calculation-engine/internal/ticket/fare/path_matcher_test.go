...
func TestPathMatcher(t *testing.T) {
	...
	fares := []ticketdomain.PathAndFare{
		{
			Path: []string{"A", "B", "C"},
			Fare:  100,
		},
	...
	t.Run("存在しない駅が含まれる場合はエラー", func(t *testing.T) {
		invalidFares := []ticketdomain.PathAndFare{
			{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotFare, gotHit := matcher.Search(tt.path)
			if gotHit != tt.wantHit {
				t.Errorf("Search() gotHit = %v, 期待値 %v", gotHit, tt.wantHit)
			}
			if gotHit && gotFare != tt.wantFare {
				t.Errorf("Search() gotFare = %v, 期待値 %v", gotFare, tt.wantFare)
			}
		})
	}
}

func TestPathMatcher_DuplicateRegistration(t *testing.T) {
	matcher := fare.NewPathMatcher()
	path := []int{1, 2, 3}
	rev := []int{3, 2, 1}
	f := 100
	...
	// 1. 初回登録（成功）
	if err := matcher.Insert(path, f); err != nil {
		t.Fatalf("初回登録に失敗しました: %v", err)
	}

	// 2. 同じ経路を再登録（エラー）
	if err := matcher.Insert(path, f); !errors.Is(err, fare.ErrDuplicateRoute) {
...
func TestPathMatcher_DefensiveCopy(t *testing.T) {
	matcher := fare.NewPathMatcher()
	path := []int{1, 2, 3}
	f := 100
...
func TestPathMatcher_HashCollision(t *testing.T) {
	matcher := fare.NewPathMatcher()
	...
	// 検証
	if f, hit := matcher.Search([]int{1, 2, 3}); !hit || f != 100 {
		t.Errorf("route1は運賃100を返すはずです, got hit=%v, fare=%v", hit, f)
	}
	if f, hit := matcher.Search([]int{3, 2, 1}); !hit || f != 100 {
		t.Errorf("route3 (route1の逆順) は運賃100を返すはずです, got hit=%v, fare=%v", hit, f)
	}
	if f, hit := matcher.Search([]int{1, 3, 2}); !hit || f != 200 {
		t.Errorf("route2は運賃200を返すはずです, got hit=%v, fare=%v", hit, f)
	}
	if f, hit := matcher.Search([]int{2, 3, 1}); !hit || f != 200 {
		t.Errorf("route2の逆順は運賃200を返すはずです, got hit=%v, fare=%v", hit, f)
	}
