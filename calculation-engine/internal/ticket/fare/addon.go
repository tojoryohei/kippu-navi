...
// AddonRegistry は特定区間の加算運賃（乗車券・大人片道）を管理します。
type AddonRegistry struct {
	definitions []addonDefinition
	resolved    map[stationPairKey]int
...
func NewAddonRegistry() *AddonRegistry {
	return &AddonRegistry{
		...
		resolved:    make(map[stationPairKey]int),
		hinenoID:    -1,
		rinkuuID:    -1,
		kansaiID:    -1,
	}
...
func (r *AddonRegistry) Register(stationA, stationB string, fare int) {
...
func (r *AddonRegistry) ResolveIDs(resolver func(string) (int, bool)) error {
	r.resolved = make(map[stationPairKey]int, len(r.definitions))
	...
	for _, def := range r.definitions {
		...
		if !okA {
			return fmt.Errorf("%w: %s", domain.ErrStationNotFound, def.StationA)
		}
		idB, okB := resolver(def.StationB)
		if !okB {
			return fmt.Errorf("%w: %s", domain.ErrStationNotFound, def.StationB)
		}
		if idA == idB {
			return fmt.Errorf("%w: %s", domain.ErrSameStation, def.StationA)
		}
...
func (r *AddonRegistry) GetApplicableAddons(path []int) []int {
	if len(path) < 2 {
		return nil
	}
