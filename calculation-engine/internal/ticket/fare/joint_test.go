...
func TestCalculateJointFare(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			components: []fare.JointFareComponent{
				...
				{CompanyID: domain.JRWest, LineType: domain.LineTypeTrunkOnly, EigyoKilo: domain.DeciKilo(305), GiseiKilo: domain.DeciKilo(305)},
			},
		...
		},
		{
			...
			components: []fare.JointFareComponent{
				...
				{CompanyID: domain.JREast, LineType: domain.LineTypeTrunkOnly, EigyoKilo: domain.DeciKilo(1005), GiseiKilo: domain.DeciKilo(1005)}, // 東日本部分 101km
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := fare.CalculateJointFare(r, tt.totalEigyo, tt.totalGisei, tt.totalLineType, tt.components)
